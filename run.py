...
topics = dict() # {topicId:{topicName:XXX;topicFollow:XXX;...};...}
topicRelationship = list() # [[topic][sonTopic]...]
try:
    file = open('zhihu-total.txt', 'r')
...
def exploreTopic(topicId=rootTopicId):
    ...
    try:
        topicData = {
            ...
            'nicks': nicks,
            'sonTopics': sonTopic,
            'top-answers': topicTop[0].get_text(strip=True),
            ...
            topics[str(topicId)] = topicData
            file = open('zhihu-total.txt', 'w')
            file.write(json.dumps(topics))
            file.close()
    finally:
        # time.sleep(0.1)
        for [name, id] in sonTopic:
            exploreTopic(id)
        topics[str(topicId)] = topicData
        print(len(topics),' | ', topicData)
        file = open('zhihu.txt', 'a')
        file.write('"'+ topicId +'":' + json.dumps(topicData)+',\n')
        file.close()
